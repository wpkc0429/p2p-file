...
- 不做病毒掃描、內容審查或檔案格式限制
- 不是完整的通訊軟體：雖提供房間內即時訊息／剪貼簿共用（見 §7.7），但不支援群組管理、已讀回條、歷史訊息保存（訊息僅存於記憶體，換房或重整即清空）
...
| `chat` | 廣播（對每個已連線 peer 各送一份） | `text`（≤ `CHAT_MAX_LEN` = 8000 字元） | 即時訊息／剪貼簿內容；與檔案交握無關，不使用 `currentRecvId`，接收端直接排入訊息列表（見 §7.7） |
...
### 7.7 訊息／剪貼簿（Chat / Clipboard）

- 與檔案傳輸**共用同一條 DataChannel mesh**：送出訊息時，對房間內每個 `dc.readyState === 'open'` 的 peer 各送一份 `{ t:'chat', text }` 文字控制訊息（廣播語意，但實作為逐一定向送出，非透過訊令伺服器）。
- 因訊息為 JSON 文字、檔案分塊為二進位，兩者在 `onDcMessage` 依型別分流，**互不干擾**；即使檔案傳輸進行中夾帶訊息，`ordered: true` 保證順序，訊息走 `onControl` 立即處理，不會被誤判為檔案分塊。
- 接收端不需同意即直接顯示（訊息無「接受/拒絕」交握），並依 `activeTab` 決定是否累加未讀數（`unread`）。
- **顯示規則**：自己送出的訊息靠右（navy→blue 漸層底、白字）、他人靠左（淺底、深字）；文字若疑似連結／程式碼／較長貼上內容（含換行、長度 > 56、`http(s)://` 開頭、email、含 `{};=<>` 等符號）則以等寬字型（`--font-mono`）呈現，方便閱讀驗證碼／片段。每則訊息附「複製」按鈕（`navigator.clipboard`，含 `execCommand` 後備）。
- 訊息長度上限 `CHAT_MAX_LEN = 8000` 字元，超過即截斷；接收端渲染一律以 `textContent` 寫入，不將訊息內容當作 HTML 注入。
- **不持久化**：訊息僅存於記憶體 `messages[]`，加入其他房間（`joinRoom`）、離開房間（`disconnect`）或重整頁面時即清空。
...
- **左欄（`.p2p-pair`）**
  ...
  - 「全網狀・零伺服器」說明卡片（靜態文案）。
- **右欄（`.p2p-files`）**：以分頁切換「檔案傳輸」與「訊息 · 剪貼簿」兩個面板，各分頁標題附未讀數徽章（`fileUnread` / `unread`，僅在該分頁非目前作用中時顯示；切換到該分頁即歸零）。
  - **檔案傳輸分頁（`#panel-files`）**
    - 拖放／點擊選檔區（`dropzone`），無連線裝置時點擊會提示「請先邀請裝置加入房間」並聚焦輸入框，而非開啟檔案選擇器。
    - 傳輸佇列標題列 + 「清除已完成」按鈕（有紀錄才顯示）。
    - 空狀態提示卡（尚無傳輸紀錄時顯示）。
    - 傳輸紀錄清單（`file-list`，新記錄插入最上方）。
  - **訊息 · 剪貼簿分頁（`#panel-chat`）**：訊息列表（`chat-list`，新訊息附加於底部並自動捲動）、輸入列（`textarea` 自動增高，Enter 送出／Shift+Enter 換行）；無已連線裝置時輸入列改為「加入房間後即可…」提示（見 §7.7）。
  - 廣告版位（AdSense 300×250 佔位，位於分頁下方常駐）。
